...
def process_sllgs_data(data_file, time_file, _t_delay, dim_points):
    ...
    print(f"max theta in data_tol {np.max(data_tol)}")
    ...
    _sllgs_rho = np.histogram2d(
        x, y, range=[None, [0, np.pi]], bins=_s_llgs_bins, density=True
    )
    print(f"histogram shape: {_sllgs_rho[0].shape}")
    # normalize and log
    _sllgs_tol_sw_idx = 0
    for h_idx, h in enumerate(_sllgs_rho[0]):
        area = np.abs(np.trapezoid(h * np.sin(theta_axis), x=theta_axis))
        _sllgs_rho[0][h_idx] = h / (2 * np.pi * area)

    _sllgs_ps_tol = np.zeros(_time_tol.shape[0])
    _sllgs_tol_sw_idx = 0
    for j in range(data_tol.shape[1]):
        _sllgs_ps_tol[j] = float(np.sum(data_tol[:, j] > np.pi / 2)) / float(
            data_tol.shape[0]
        )
    ...
    # re-make the tupple
    _sllgs_rho = ((_sllgs_rho[0].T)[:, rho_0_time_idx:], _sllgs_rho[1], _sllgs_rho[2])
    print(f"H_tol shape: {_sllgs_rho[0].shape}")
