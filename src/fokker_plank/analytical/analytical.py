...
def sph_area_rho_over_theta(rho, theta, axis=-1):
    ...
    """
    return np.abs(np.trapezoid(rho * np.sin(theta), x=theta))
...
def generate_A(i, h, delta, j):
    ...
    A[j - 1, j - 1] = (
        -1.0 * (j - 1) * j * (inv_2_delta - 1 / ((2 * j + 1) * (2 * j - 3)))
    )
...
def get_state(tau, i=None, h=None, delta=None, state_0=None, a_matrix=None):
    ...
    return np.matmul(
        scipy.linalg.expm(tau * generate_A(i, h, delta, state_0.size - 1)), state_0
    )


def untangle_state(m_state, lin_space_z=False, dim_points=2000):
...
def get_time_to_sw(
    ...
    while True:
        ...
        if do_manual_sw_prob:
            ...
            sw_prob = compute_sw_prob(
                data=data, theta=theta, rho_0_at_pi=rho_0_at_pi, normalize=True
            )
        else:
    ...
    while iteration < max_iteration:
        ...
        if iteration > 1 and iteration % 50 == 1:
            print(
                f"iteration: {iteration}, t: {t}, sw_prob: {sw_prob}"
...
def get_time_to_sw_fitting(
    c, delta, nu, alpha, h_k, temperature=300, rho_0_at_pi=False
):
    """
    ...
    i_c = (
        delta
        * (4 * alpha * sllgs_solver.c_E * sllgs_solver.c_KB * temperature)
        / (nu * sllgs_solver.c_hbar)
    )
    t_d = (1 + alpha * alpha) / (
...
def basic_error_fn(error_mode, _get_time_to_sw_fitting, currents, _times):
    ...
    if error_mode == 0:
        ...
        def err(p):
            return np.mean((_get_time_to_sw_fitting(currents, *p) - _times) ** 2)
    elif error_mode == 2:
        print("[fitting] Error mode 2: doing abs(err/x)")

        def err(p):
            return np.mean(
                np.abs((_get_time_to_sw_fitting(currents, *p) - _times) / _times)
            )
    elif error_mode == 3:
        print("[fitting] Error mode 3: doing abs(err)")

        def err(p):
            return np.mean(np.abs((_get_time_to_sw_fitting(currents, *p) - _times)))
...
def _minimize_error(err, minimize_mode, p0, bounds):
    ...
    if minimize_mode == 0:
        ...
        # shgo
        res = optimize.shgo(err, bounds=bounds, iters=200)
        print(f"[res] Mode {minimize_mode}: {res}")
        popt = res.x
        return popt
        ...
        # shgo
        res = optimize.brute(err, ranges=bounds, finish=optimize.fmin)
        print(f"[res] Mode {minimize_mode}: {res}")
...
def get_state_rho_0(
    ...
    if do_maxwell:
        ...
        # fitted
        rho_0 = maxwell.pdf(theta, loc=maxwell_loc, scale=maxwell_scale)
    else:
        sin_theta = np.sin(theta)
        rho_0 = np.exp(-delta * sin_theta * sin_theta) * np.heaviside(
            np.pi / 2 - theta, 0.5
        )
    ...
    s_rho_0 = np.polynomial.legendre.legfit(x=z, y=rho_0, deg=L0)
...
def get_sn(L0):
    ...
    # factorials
    nf = np.array(
        [scipy.special.factorial2(f, exact=True) for f in sn[3::2].astype(fact_type)]
    ).astype(dtype)
    nfm1 = np.array(
        [scipy.special.factorial2(f, exact=True) for f in sn[2:-1:2].astype(fact_type)]
    ).astype(dtype)
    # odd numbers
...
def compute_sw_prob(data, theta, rho_0_at_pi, normalize=False):
    ...
    #     #       f'{2*np.pi*np.trapezoid(data, np.cos(theta))}')
    #     first_theta = theta[0]
    ...
    if rho_0_at_pi:
        return (
            2
            * np.pi
            * sph_area_rho_over_theta(
                data[:, theta < np.pi / 2], theta[theta < np.pi / 2]
            )
        )
    return (
        2
        * np.pi
        * sph_area_rho_over_theta(data[:, theta > np.pi / 2], theta[theta > np.pi / 2])
    )
...
def get_sw_prob(
    ...
    if compute_analytical_manual:
        manual_prob = compute_sw_prob(
            data=data, theta=theta, rho_0_at_pi=rho_0_at_pi, normalize=True
        )
    else:
        manual_prob = -1
    if compute_fvm:
        theta, rho_init = untangle_state(
            s_rho_0, dim_points=dim_points, lin_space_z=lin_space_z
        )
...
def get_sw_continuous_prob(
    ...
    for t_idx, t in enumerate(time):
        ...
        if compute_analytical_manual:
            manual_prob[t_idx] = compute_sw_prob(
                data=data, theta=theta, rho_0_at_pi=rho_0_at_pi, normalize=True
            )
        else:
            manual_prob[t_idx] = -1
    ...
    if compute_fvm:
        ...
        fvm_prob = compute_sw_prob(
            data=data, theta=fvm_theta, rho_0_at_pi=rho_0_at_pi, normalize=True
        )
    else:
