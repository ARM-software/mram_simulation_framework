...
HEXT = (0.0, 0.0, 0.0)  # low external magnetic field (A/m) (test anisotropy)
# stt params
...
MARKERS = itertools.cycle(("o", "*", "d", "1", ",", "+", ".", "s", "X", "x"))
...
print(f"P: {P}, P/2: {P / 2}, eps: {LLG_O[0].get_epsilon_stt(1)}")
...
def analyze_s_llgs_tolerances(
    ...
    _, _, s_rho_0_b = analytical.get_state_rho_0(
        delta=delta,
        do_maxwell=False,
        L0=Nmax,
    ...
    fp_theta, ddata_fit = analytical.untangle_state(
        s_rho_0_fit, dim_points=dim_points, lin_space_z=lin_space_z
    )
    fp_theta, ddata_a = analytical.untangle_state(
        s_rho_0_a, lin_space_z=lin_space_z, dim_points=dim_points
    )
    fp_theta, ddata_b = analytical.untangle_state(
        s_rho_0_b, lin_space_z=lin_space_z, dim_points=dim_points
    )

    fig, axs = plt.subplots(1, 1, figsize=(8, 6))
    axs.plot(theta_axis, sllgs_data[0][:, rho_0_time_idx][::-1], "+-", label="sllgs")
    axs.plot(fp_theta, ddata_fit, label="fit")
    axs.plot(fp_theta, ddata_a, "--", label="A")
    axs.plot(fp_theta, ddata_b, "--", label="B")
    axs.grid()
    axs.legend()
    ...
    fp_a_switching_idx = np.min([len(fp_a_ps) - 1, np.searchsorted(fp_a_ps, 0.5)])
    fp_b_switching_idx = np.min([len(fp_b_ps) - 1, np.searchsorted(fp_b_ps, 0.5)])
    ...
    if False and plot_intermediate:
        ...
        ax00 = ax0.twinx()
        ax00.plot(
            [1e9 * sllgs_time[sllgs_sw_idx], 1e9 * sllgs_time[sllgs_sw_idx]],
            [0, 1],
            "--",
            color="tomato",
        )
        ax00.plot([1e9 * (t_pulse), 1e9 * (t_pulse)], [0, 1], "--", color="tomato")
        # axx.plot([0, 1e9*time_tols[-1]],
        #          [0.5, 0.5],
        #          '--', color='tomato')
        ax00.plot(1e9 * sllgs_time, sllgs_sw_ps, "--", color="whitesmoke")
        # ax11.grid()
        ax00.set_ylabel(r"Switching probability")
        ax00.annotate(
            f"sw @ {1e9 * sllgs_time[sllgs_sw_idx]:.2f} ns",
            xy=(1e9 * sllgs_time[sllgs_sw_idx], 0.5),
            xytext=(1e9 * sllgs_time[sllgs_sw_idx] + 1, 0.5),
            color="whitesmoke",
        )
        ax00.annotate(
            f"write ends @ {1e9 * (t_pulse):.2f} ns",
            xy=(1e9 * (t_pulse), 0.5),
        ...
        ax2.plot(
            [1e9 * fp_time[fp_a_switching_idx], 1e9 * fp_time[fp_a_switching_idx]],
            [0, np.pi],
            "--",
            color="tomato",
        )
        ax22 = ax2.twinx()
        ax22.plot(1e9 * fp_time, fp_a_ps, "--", color="whitesmoke")
        ax22.set_ylabel(r"Switching probability")
        ax22.plot([1e9 * (t_pulse), 1e9 * (t_pulse)], [0, 1], "--", color="tomato")
        ...
        ax3.plot(
            [1e9 * fp_time[fp_b_switching_idx], 1e9 * fp_time[fp_b_switching_idx]],
            [0, np.pi],
            "--",
            color="tomato",
        )
        ax22 = ax3.twinx()
        ax22.plot(1e9 * fp_time, fp_b_ps, "--", color="whitesmoke")
        ax22.plot([1e9 * (t_pulse), 1e9 * (t_pulse)], [0, 1], "--", color="tomato")
        ax22.set_ylabel(r"Switching probability")
        ...
        ax2.annotate(
            f"sw @ {1e9 * fp_time[fp_a_switching_idx]:.2f} ns",
            xy=(1e9 * fp_time[fp_a_switching_idx], 0.5),
            xytext=(1e9 * fp_time[fp_a_switching_idx] + 1, 0.5),
            color="whitesmoke",
        )
        ax3.annotate(
            f"sw @ {1e9 * fp_time[fp_b_switching_idx]:.2f} ns",
...
def plot_results(
    ...
    for idx in range(len(sllgs_files)):
        ...
        marker = next(MARKERS)
        np.savetxt(
            f"dummy_sllgs_wer_{idx}.csv", np.array([time_sslgs[idx], 1 - ps_sslgs[idx]])
        )
        ax.plot(
            1e9 * (time_sslgs[idx]) - t_delays_ns[idx],
            1 - ps_sslgs[idx],
